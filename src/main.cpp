...
int main(){
    ...
    prntd(sig.as_vector_const());
    prntd(th.as_vector_const());

    std::cout<<"Conc vector ";
    Tensor m = con[1].mean(1);
    prntd(con[1].as_vector_const());
    ...
    prntd(a->get_data().as_vector_const());
    ...
    prntd(z->get_grad().as_vector());

    std::cout<<"b data output"<<std::endl;
    prntd(b->get_data().as_vector());

    std::cout<<"l data output"<<std::endl;
    prntd(l->get_data().as_vector());

    std::cout<<"a grad output"<<std::endl;
    prntd(a->get_grad().as_vector());
